...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                LingManager.getInstance().getLingLog().LOGD("LocalBroadcastManager", "Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    LingManager.getInstance().getLingLog().LOGD("LocalBroadcastManager", "Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        LingManager.getInstance().getLingLog().LOGD("LocalBroadcastManager", "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            LingManager.getInstance().getLingLog().LOGD("LocalBroadcastManager", "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                LingManager.getInstance().getLingLog().LOGD("LocalBroadcastManager", "  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            LingManager.getInstance().getLingLog().LOGD("LocalBroadcastManager", "  Filter did not match: " + reason);
                        }
                    }
                }
